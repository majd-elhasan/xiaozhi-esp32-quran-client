...
This sketch streams an MP3 stored on an SD card over the MAX98357A I²S amplifier. It runs on the ESP32 core, so the `SD` and `SPI` headers are provided by `esp32:esp32`.
...
* **Serial commands** (enter them into the Sketch Serial Monitor):
  * `play stop <surah> <ayah>` — play the requested ayah and stop when it ends.
  * `play continue <surah> <ayah>` — play from that ayah through the end of the surah, wait `surahEndDelayMs` (default 10 s), and then automatically begin the next surah.
...
* **MCP tools** (exposed once the sketch connects to `MCP_ENDPOINT`):
  * `play_ayah` — accepts `surah`, `ayah`, and an optional `mode` (`stop` or `continue`) and triggers the same playback flows as the serial helper.
  * `stop_playback` — immediately cancels playback.
